...
function weightCalc(id, weightMult){
    ...
    newWeight = (Math.round(Math.random() * max)+min)*weightMult
    ...
    if (controlsVisibility) {
        ...
        if (key["ArrowUp"] && Date.now() > (lastFishMove + 250)) {
            ...
            if (FV){
                ...
                splash.style.backgroundImage = `url('char/bob.png')`

                setTimeout(() => {
                    id = Math.round(Math.random() * lootpool[gridData[map][PY+FY][PX+FX].loot].length)

                    if (Math.round(Math.random() * 32) == 1) {
                        fishType = "R"
                    } else {
                        fishType = ""
                    }
