...
try{
    if (saveData != null){
        ...
        if (saveData.auto != undefined){
            auto = saveData.auto
        }

        if (saveData.cashMoney != undefined){
            cashMoney = saveData.cashMoney
        }

        if (saveData.last != undefined){
            last = saveData.last
        }

        if (saveData.unlocked != undefined){
            unlocked = saveData.unlocked
        }

        if (saveData.infinity != undefined){
            infinity = saveData.infinity
        }
...
function addButton(id, dataless = false){
    ...
    newbutton.className = `button autoButton${id}`
    ...
    temp[5].className = `button autobuy${id}`
...
function gameloop(){
    cashMoney += ((auto[0].count*((Date.now()-last)/10000))*auto[0].mult)*(infinity+1)
    ...
    if (auto[unlocked].count != 0 && unlocked < 19){
        unlocked++

        addButton(unlocked)
    }

    if (cashMoney == Infinity){
        infinity += 1
    ...
    if ((lastSave+1000) < Date.now()){
        localStorage.setItem("savedata", JSON.stringify({"auto": auto, "cashMoney": cashMoney,  "infiniy": infinity, "last": last, "unlocked": unlocked}))
    }
}

function kill(name){
    clearInterval(name)
    localStorage.setItem("savedata", JSON.stringify({"infiniy": infinity}))
